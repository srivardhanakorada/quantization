    ...
    def forward(self, x):
        ...
        if isinstance(self.quant_hub_layer.core, torch.nn.Conv2d):
            ...
            out = F.conv2d(
                ...
                w,
                bias,
                self.quant_hub_layer.core.stride,
                self.quant_hub_layer.core.padding,
                self.quant_hub_layer.core.dilation,
                self.quant_hub_layer.core.groups,
            )
            ...
            if bias is not None:
                bias = bias.to(compute_dtype)
            out = F.linear(x.to(compute_dtype), w, bias)
            return out.to(origin_dtype)
        else:
